...
// ── 커리큘럼 필터링된 카드 목록 ──────────────────
let availableCards = VOCAB_CARDS;
...
export function initVocab(userKey = "default") {
  ...
  availableCards   = VOCAB_CARDS.filter(c => availCats.includes(c.category));
...
function loadCards() {
  ...
  allCards = availableCards.map(word => ({
    ...word,
...
function renderVocabPage() {
  ...
  page.innerHTML = `
    <link rel="stylesheet" href="public/css/vocab.css?v=17" />

    <div class="page-header">
      <button class="page-back-btn" id="vc-home-btn">‹</button>
      <div class="page-header-info">
        <div class="page-header-title">어휘 플래시카드</div>
        <div class="page-header-sub" id="vc-sub">오늘 카드</div>
      </div>
    </div>
  ...
  document.getElementById("vc-home-btn")?.addEventListener("click", () => window.navigateTo("curriculum"));
  bindVocabEvents();
}
